...
const PricingCard = ({ plan, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl p-6 md:p-8 border-2 flex flex-col relative overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 ${plan.borderColor} ${plan.bgColor} ${plan.popular ? 'ring-4 ring-offset-2 ring-offset-background dark:ring-offset-slate-900 ring-yellow-400 dark:ring-yellow-500' : ''}`}
    >
      {plan.popular && (
        ...
        <ul className="space-y-3 mb-8 flex-grow">
          {plan.features.map((feature, i) => (
            <li key={i} className={`flex items-start ${plan.textColor === 'text-white' || plan.textColor.includes('dark:text-white') ? 'text-gray-200' : 'text-foreground/90'}`}>
              <CheckCircle size={18} className={`${plan.popular ? 'text-yellow-400 dark:text-yellow-500' : 'text-green-500 dark:text-green-400'} mr-3 mt-0.5 flex-shrink-0`} />
              <span>{feature}</span>
            </li>
          ))}
        </ul>
      </div>
      <Button className={`w-full ${plan.buttonClass} text-base font-semibold py-3 transition-transform duration-200 hover:scale-105 ${plan.name === 'Profesional' ? 'shadow-lg shadow-green-500/30' : ''} ${plan.popular ? 'shadow-lg shadow-primary/30 dark:shadow-blue-500/30' : ''}`}>
        {plan.buttonText}
      </Button>
    </motion.div>
  );
